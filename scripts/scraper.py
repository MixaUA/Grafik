...
# Створюємо "розумний" скрапер, який обходить захист Cloudflare
...
def parse_queue_page(q_id):
    ...
    try:
        ...
        if response.status_code == 403:
            print(f"❌ Доступ заблоковано (403) для черги {q_id}")
            return None
        ...
        for h in headers:
            ...
            if "періоди відключень" in txt:
                ...
                if ul:
                    ...
                    if current_page_date and periods:
...
def main():
    ...
    final_queues = {q: {d: [] for d in DAYS_UA} for q in queues}
    success_count = 0
    ...
    for q in queues:
        ...
        if parsed is None: # Якщо отримали 403
            continue
            
        for day, p in parsed.items():
            final_queues[q][day] = p
            success_count += 1
        
        time.sleep(random.uniform(3, 7))

    # Запобіжник: якщо ми не змогли зібрати дані хоча б для однієї черги через 403
    if success_count == 0:
        print("🛑 КРИТИЧНА ПОМИЛКА: Не вдалося отримати дані з жодної сторінки (всі 403).")
        sys.exit(1) # Зупиняємо екшен, щоб не ламати базу
    ...
    print(f"✨ Успішно оновлено {success_count} графіків!")
