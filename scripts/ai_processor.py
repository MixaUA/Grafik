...
def main():
    ...
    try:
        ...
        prompt = (
            ...
            "1. Визнач дату (DD.MM.YYYY) та день тижня українською.\n"
            "2. Витягни інтервали відключень для підчерг 1.1-6.2.\n"
            "Поверни JSON: {'date': '...', 'day_of_week': '...', 'queues': {'1.1': ['00:00-02:00'], ...}}"
        )
        ...
        if json_match:
            ...
            target_day = res.get('day_of_week', '').lower()
            
            if target_day not in ua_days: target_day = today_name

            # Очистка: створюємо новий скелет черг лише для сьогодні і завтра
            all_q_names = [f"{i}.{j}" for i in range(1, 7) for j in range(1, 3)]
            new_queues = {q: {d: [] for d in ua_days} for q in all_q_names}
            
            # Переносимо дані старого дня, якщо вони ще актуальні (сьогодні або завтра)
            if "queues" in db:
            ...
            print(f"🎉 Графік на {display_date} оновлено!")
