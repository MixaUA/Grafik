...
def build_weather_summary_prompt(morning, afternoon, evening, date_str, day_word,
    ...
    if temp_min is not None and temp_max is not None:
        range_note = (
            f"\nТемпературна крива доби: мінімум {ft(temp_min)} — вранці, "
            f"пік {ft(temp_max)} — вдень, ввечері спад нижче піку. "
            f"Це залізне правило: вечір ЗАВЖДИ холодніший за день. "
            f"Використовуй ТІЛЬКИ ці цифри — не вигадуй інших значень!"
        )
...
- Температурна логіка: вранці холодно, вдень пік, ввечері спад — ніколи не навпаки
...
def build_weather_summary_prompt_two_days(
    ...
    def range_note(mn, mx, label):
        if mn is not None and mx is not None:
            return (
                f"\nТемпературна крива {label}: мінімум {ft(mn)} — вранці, "
                f"пік {ft(mx)} — вдень, ввечері спад нижче піку. "
                f"Вечір ЗАВЖДИ холодніший за день — це залізне правило. "
                f"Використовуй ТІЛЬКИ ці цифри — не вигадуй інших!"
            )
        return ""
